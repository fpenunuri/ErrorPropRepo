%FVAJ = KinQ03(f,q3p,q2p,q1p,q0p,pars)
%output variables
%FVAJ: tensor with the n-th order derivative of f; n=0,1,2,3
%
%input variables
...
function fr = KinQ03(f,q3p,q2p,q1p,q0p,pars)
  taux = dual3(0,1,0,0);
  Xt = q0p + taux.*q1p + taux.^2 .* q2p./2 + taux.^3 .* q3p./6;
  fr = f(Xt,pars);
  rango = numel(size(fr));
  fr = cat(rango+1,fr.f0,fr.f1,fr.f2,fr.f3);
end
