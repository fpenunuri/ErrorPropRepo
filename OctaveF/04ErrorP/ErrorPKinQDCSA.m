%[DF,DV,DA,DJ,DS] = ErrorPKinQDCSA(f,q4p,q3p,q2p,q1p,q0p,parv,dq0p,dparv)
%output variables
...
%DS: error propagation on the jounce/snap vector
%
%Input variables
...
%   of time
%q4p,q3p,q2p,q1p,q0p: vectors that store, the time derivatives of the 
...
%
%This function is vectorized in qkp,parv,dparv (although f will 
%dictate the kind of arguments), k=0,1,2,3,4
%
function [DF,DV,DA,DJK,DJS] = ErrorPKinQDCSA(f,q4p,q3p,q2p,q1p,q0p,...
                                             parv,dq0p,dparv)
  ...
  suma = (dparv(:,1).*imag(KinQ04(f,q4p,q3p,q2p,q1p,q0p,parv+ihm)/h)).^2;
  for k=2:n
    ihm = cerom;
    ihm(:,k) = ih;
    suma = suma + (dparv(:,k).*...
                   imag(KinQ04(f,q4p,q3p,q2p,q1p,q0p,parv+ihm))/h).^2;
  end
  ...
  suma = suma + ...
         (dq0p(:,1).*imag(KinQ04(f,q4p,q3p,q2p,q1p,q0p+ihm,parv)/h)).^2;

  for k=2:n
    ihm = cerom;
    ihm(:,k) = ih;
    suma = suma + (dq0p(:,k).*...
                   imag(KinQ04(f,q4p,q3p,q2p,q1p,q0p+ihm,parv))/h).^2;
  end
